        ...
        for t in range(config.MAX_TIMESTEPS):
            ...
            for obs_center_2d, obs_radius, _ in obstacle_list:
                ...
                if newly_occluded_local_indices.size > 0:
                    ...
                    # Remove the newly occluded points from the main list for the *next* iteration.
                    visible_indices = np.setdiff1d(
                        visible_indices, newly_occluded_global_indices, assume_unique=True
                    )

            # After the loop, visible_indices contains the final list of all non-occluded points.
            ...
            crashed = False
            for obs_center_2d, obs_radius, _ in obstacle_list:
                if np.linalg.norm(state_vec - obs_center_2d) < obs_radius:
                    crashed = True
                    break
            ...
            if timestep % config.BATCH_UPDATE_TIMESTEP == 0:
                ...
                update_step_count += 1
                memory.clear()
                # You should not reset timestep here if it's the main loop counter.
                # Let the outer loop handle it. If you need a separate counter
                # for batches, reset that one. Based on your code, resetting
                # timestep to 0 is what you want, so we'll keep it.
                timestep = 0
