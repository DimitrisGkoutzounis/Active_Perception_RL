...
def show_map_multiple_obstacles(obstacle_list, mu, pcd3d, d_min, trajectory=None, reward_map=None, reward_map_extent=None):
    """Displays the top-down map with ROI, obstacles, and agent trajectory."""
    clicked_point = []
    ...
    # Plot Point Cloud (ROI) and its center
    ...
    # Add guide circles from original script
    ...
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    # ax.legend() # Original was commented out, so I'll respect that
...
def plot_run_diagnostics(rewards, trajectory, n_steps):
    """Plots the reward and agent coordinates over time for a single run."""
...
def plot_frame_comparison(step_title, gnt_points, obs_points, H_gnt, H_obs, image_w, image_h, hist_range, gnt_intensity=None, obs_intensity=None):
    """2x2 comparison of points and heatmaps for ground truth vs. observed."""
...
# The generate_reward_map function does not need changes.
